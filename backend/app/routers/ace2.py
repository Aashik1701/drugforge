"""
ACE2 binding prediction router.

Endpoint: POST /predict/ace2
"""
...
def _get_model():
    ...
    return model_loader.get("ace2")


@router.post("/ace2", response_model=PredictionResponse)
async def predict_ace2(mol: MoleculeInput) -> PredictionResponse:
    """
    Predict ACE2 receptor binding probability.

    Args:
        mol: Request body with a SMILES string.

    Returns:
        PredictionResponse with ACE2 binding probability.
    ...
    model = _get_model()
    if model is None:
        raise HTTPException(status_code=503, detail="ACE2 model is not available")
    ...
    try:
        ...
        logger.error(f"ACE2 inference failed: {e}")
