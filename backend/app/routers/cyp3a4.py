"""
CYP3A4 inhibition prediction router.

Endpoint: POST /predict/cyp3a4
"""
...
def _get_model():
    ...
    return model_loader.get("cyp3a4")


@router.post("/cyp3a4", response_model=PredictionResponse)
async def predict_cyp3a4(mol: MoleculeInput) -> PredictionResponse:
    """
    Predict CYP3A4 enzyme inhibition probability.

    Args:
        mol: Request body with a SMILES string.

    Returns:
        PredictionResponse with inhibition probability.
    ...
    model = _get_model()
    if model is None:
        raise HTTPException(status_code=503, detail="CYP3A4 model is not available")
