...
def _get_model():
    ...
    return model_loader.get("toxicity")


@router.post("/toxicity", response_model=PredictionResponse)
async def predict_toxicity(mol: MoleculeInput) -> PredictionResponse:
    """
    Predict general toxicity probability.
    ...
    model = _get_model()
    if model is None:
        raise HTTPException(status_code=503, detail="Toxicity model is not available")
